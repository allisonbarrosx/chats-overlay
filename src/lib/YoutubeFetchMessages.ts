...
import { proxies, type ChatMessage, type YoutubeChatResponse } from '../shared/constants.js';
...
async function fetchYoutubeLiveChatMessages(userChannel: string): Promise<void | boolean> {
	const youtubeLiveInfo = get(youtubeLiveInfoStore);
	console.log('youtubeLiveInfo', youtubeLiveInfo);

	if (youtubeLiveInfo.isChannelLive === true && youtubeLiveInfo.liveId !== null) {
...
async function fetchYoutubeMessagesFromApi() {
	const youtubeLiveInfo = get(youtubeLiveInfoStore);
	try {
		const response = await fetch(`/api/live-chat?videoId=${youtubeLiveInfo.liveId}`);
		const data = await response.json();
		if (data.error) {
			console.log('Error while getting live chat messages: ', data.error);
		} else {
			data.forEach(
				(item: YoutubeChatResponse) => {
					const randomColor = `#${Math.floor(Math.random() * 16777215).toString(16)}`;
					chatStore.addMessage({
